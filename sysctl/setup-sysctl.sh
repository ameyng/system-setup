  ...
  for config_file in "${script_directory_path}/"*.conf; do
    ...
    if [ -f "${config_file}" ]; then
      ...
      if [ -L "/etc/sysctl.d/${config_file_name}" ]; then
        ...
        # Unlink it.
        echo "'/etc/sysctl.d/${config_file_name}' exists as a symbolic link, unlinking it."
        unlink "/etc/sysctl.d/${config_file_name}"
      ...
      # Otherwise, check if it is a regular file.
        ...
        echo "'/etc/sysctl.d/${config_file_name}' exists, creating backup."
        mv -f "/etc/sysctl.d/${config_file_name}" "/etc/sysctl.d/${config_file_name}.backup.${timestamp_now}"
      ...
      echo "Installing '${config_file}' into '/etc/sysctl.d'"
